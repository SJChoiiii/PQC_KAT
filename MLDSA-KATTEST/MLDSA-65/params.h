...
#define SEEDBYTES 32            // 256bit 짜리 random 값을 구할 때 이용 & A, s1, s2, MLDSA65_K 의 seed를 생성하는 seed, 
...
#define MLDSA65_N 256         // 다항식의 최대 차수
#define Q 8380417               // modulo Q
#define D 13                    // t = t1*2^D+t0 를 구할 때 사용하는 즉, t의 high bit를 구하기 위한 값
#define ROOT_OF_UNITY 1753      // NTT에서 이용하는 MLDSA65의 root of unity

#define MLDSA65_K 6           // 공개행렬 A에서 행의 크기
#define MLDSA65_L 5           // 공개행렬 A에서 열의 크기
#define ETA 4                   // s1, s2의 범위
#define TAU 49                  // c 벡터에서 계수의 크기가 +1, -1인 계수의 개수
#define BETA 196                 // eta * tau
#define GAMMA1 (1 << 19)        // y 행렬의 계수의 범위
#define GAMMA2 ((Q-1)/32)       // t를 제외한 high bit를 구하기 위한 범위 (보통 2*gamma를 이용)        
#define OMEGA 55                // hint vector h에서의 최대 1의 개수 -> 하나의 다항식에서의 최대 1의 개수가 아니라는 점을 조심
                                // 이 omega의 제약은 BitPack, BitUnpack에서 더 효율적으로 인코딩 및 디코딩을 할 수 있도록 함


#define CTILDEBYTES 48          // tilde c의 범위 설정
...
각 계수들은 MLDSA65의 보안레벨에 따라 달라지기 때문에 packing 할 때의 크기 역시 보안 레벨에 따라 달라지게 됨
*/


#define POLY_UNIFORM_NBLOCKS ((768 + STREAM128_BLOCKBYTES - 1)/STREAM128_BLOCKBYTES)                            // MLDSA65의 계수는 23bit 표현이기 때문에, (256 * 24 / 8 +  168 - 1) / 168 = 5
...
#define POLYETA_PACKEDBYTES  128                 // s1, s2       3bit 표현의 계수 256개로 이루어져 있음. 256 * 3/8 byte 

#define POLYVECH_PACKEDBYTES (OMEGA + MLDSA65_K)        // h            hint vector h는 계수가 1 또는 0이기 때문에 일반적인 encoding과 다를 수 있음. 
...
#define PQCLEAN_MLDSA65_CLEAN_CRYPTO_PUBLICKEYBYTES (SEEDBYTES + MLDSA65_K*POLYT1_PACKEDBYTES)
...
#define PQCLEAN_MLDSA65_CLEAN_CRYPTO_SECRETKEYBYTES (2*SEEDBYTES \
        + TRBYTES \
        + MLDSA65_L*POLYETA_PACKEDBYTES \
        + MLDSA65_K*POLYETA_PACKEDBYTES \
        + MLDSA65_K*POLYT0_PACKEDBYTES)

#include <stdint.h>

#define PQCLEAN_MLDSA65_CLEAN_CRYPTO_BYTES (CTILDEBYTES + MLDSA65_L*POLYZ_PACKEDBYTES + POLYVECH_PACKEDBYTES)  // packing 했을 때의 signature의 크기:  challenge 벡터를 생성하는 seed + z의 다항식 벡터 + h의 다항식 벡터

typedef struct {
        int32_t coeffs[MLDSA65_N];
} poly;
