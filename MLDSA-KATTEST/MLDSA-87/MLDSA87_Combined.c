...
int PQCLEAN_MLDSA87_CLEAN_crypto_sign_keypair_KAT(uint8_t *pk, uint8_t *sk, uint8_t* xi) {
    uint8_t seedbuf[2 * SEEDBYTES + CRHBYTES];
    uint8_t tr[TRBYTES];
    const uint8_t *rho, *rhoprime, *key;
    polyvecl mat[MLDSA87_K];
...
int PQCLEAN_MLDSA87_CLEAN_crypto_sign_signature_ctx_KAT(uint8_t *sig,
        size_t *siglen,
        const uint8_t *m,
        size_t mlen,
        const uint8_t *ctx,
...
int PQCLEAN_MLDSA87_CLEAN_crypto_sign_ctx_KAT(uint8_t *sm,
        size_t *smlen,
        const uint8_t *m,
        size_t mlen,
        const uint8_t *ctx,
    ...
    ret = PQCLEAN_MLDSA87_CLEAN_crypto_sign_signature_ctx_KAT(sm, smlen, sm + PQCLEAN_MLDSA87_CLEAN_CRYPTO_BYTES, mlen, ctx, ctxlen, sk, rng);
    *smlen += mlen;
    return ret;
}
...
int MLDSA_KAT_TEST_DET()
{
    ...
    size_t mlen, smlen;
    uint8_t b;
    uint8_t *m;  //[MLEN + PQCLEAN_MLDSA87_CLEAN_CRYPTO_BYTES]; //= {0x6D, 0xBB, 0xC4, 0x37, 0x51, 0x36, 0xDF, 0x3B, 0x07, 0xF7, 0xC7, 0x0E, 0x63, 0x9E, 0x22, 0x3E};    // 실제         message
    uint8_t *m2; //[MLEN + PQCLEAN_MLDSA87_CLEAN_CRYPTO_BYTES];   // verify에 있는 message
    uint8_t *sm;//[MLEN + PQCLEAN_MLDSA87_CLEAN_CRYPTO_BYTES];   // sign에 있는  message
    uint8_t pk[PQCLEAN_MLDSA87_CLEAN_CRYPTO_PUBLICKEYBYTES];
    uint8_t sk[PQCLEAN_MLDSA87_CLEAN_CRYPTO_SECRETKEYBYTES];
    ...
    uint8_t pk_temp[PQCLEAN_MLDSA87_CLEAN_CRYPTO_PUBLICKEYBYTES];
    uint8_t sk_temp[PQCLEAN_MLDSA87_CLEAN_CRYPTO_SECRETKEYBYTES];
    ...
    FILE* fp;
    fp = fopen("kat_MLDSA_87_det_pure.rsp", "r");
    while(feof(fp) == 0)
    {
        ...
        get_kat_parameter(fp, pk_temp, PQCLEAN_MLDSA87_CLEAN_CRYPTO_PUBLICKEYBYTES);
        get_kat_parameter(fp, sk_temp, PQCLEAN_MLDSA87_CLEAN_CRYPTO_SECRETKEYBYTES);
        msg = (uint8_t*)malloc(mlen_temp * (count_temp + 1));
        m = (uint8_t*)malloc(mlen_temp * (count_temp + 1) + PQCLEAN_MLDSA87_CLEAN_CRYPTO_BYTES);
        m2 = (uint8_t*)malloc(mlen_temp * (count_temp + 1) + PQCLEAN_MLDSA87_CLEAN_CRYPTO_BYTES);
        ...
        sm_temp = (uint8_t*)malloc(PQCLEAN_MLDSA87_CLEAN_CRYPTO_BYTES + mlen_temp * (count_temp + 1));
        sm = (uint8_t*)malloc(PQCLEAN_MLDSA87_CLEAN_CRYPTO_BYTES + mlen_temp * (count_temp + 1));
        get_kat_parameter(fp, sm_temp, PQCLEAN_MLDSA87_CLEAN_CRYPTO_BYTES + mlen_temp * (count_temp + 1));
        ...
        PQCLEAN_MLDSA87_CLEAN_crypto_sign_keypair_KAT(pk, sk, xi);
        if(memcmp(pk, pk_temp, PQCLEAN_MLDSA87_CLEAN_CRYPTO_PUBLICKEYBYTES) != 0) return -1;
        if(memcmp(sk, sk_temp, PQCLEAN_MLDSA87_CLEAN_CRYPTO_SECRETKEYBYTES) != 0) return -1;

        memcpy(m, msg, mlen_temp * (count_temp + 1));
        PQCLEAN_MLDSA87_CLEAN_crypto_sign_ctx(sm, &smlen, m, mlen_temp * (count_temp + 1), ctx_temp, CTX_LEN, sk);
        ...
        //     if(i == CTILDEBYTES - 1)
        //         printf("\n\n");
        //     if(i == CTILDEBYTES + MLDSA87_L*POLYZ_PACKEDBYTES -1)
        //         printf("\n\n");
        //     if(i == CTILDEBYTES + MLDSA87_L*POLYZ_PACKEDBYTES + POLYVECH_PACKEDBYTES - 1)
        //         printf("\n\n");
        // }
        // printf("\n\n");
        // printf("sm temp is\n\n");
        ...
        ret = PQCLEAN_MLDSA87_CLEAN_crypto_sign_open_ctx(m2, &mlen, sm, smlen, ctx_temp, CTX_LEN, pk);
        ...
        if (smlen != mlen_temp * (count_temp + 1) + PQCLEAN_MLDSA87_CLEAN_CRYPTO_BYTES)
        {
            fprintf(stderr, "Signed message lengths wrong\n");
            return -1;
        }
...
int MLDSA_KAT_TEST_HEDGED()
{
    ...
    FILE* fp;
    fp = fopen("kat_MLDSA_87_hedged_pure.rsp", "r");
    while(feof(fp) == 0)
    {
        get_kat_parameter(fp, &count_temp, 1);
        get_kat_parameter(fp, xi, SEEDBYTES);
        ...
        memcpy(m, msg, mlen_temp * (count_temp + 1));
        PQCLEAN_MLDSA87_CLEAN_crypto_sign_ctx_KAT(sm, &smlen, m, mlen_temp * (count_temp + 1), ctx_temp, CTX_LEN, sk, rng);
