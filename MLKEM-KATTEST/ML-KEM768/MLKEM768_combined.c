...
void PQCLEAN_MLKEM768_CLEAN_cmov_int16(int16_t *r, int16_t v, uint16_t b) { // coef, ( q+1 / 2 ), msg bit
...
void PQCLEAN_MLKEM768_CLEAN_kyber_shake128_absorb(xof_state *state, const uint8_t seed[KYBER_SYMBYTES], uint8_t x, uint8_t y) {
...
void PQCLEAN_MLKEM768_CLEAN_poly_frommsg(poly *r, const uint8_t msg[KYBER_INDCPA_MSGBYTES]) { // m을 m*로 변경해주는 함수 -> 0 : 0, 1 : ( (q + 1) / 2 )
    size_t i, j;

    for (i = 0; i < KYBER_N / 8; i++) {
        for (j = 0; j < 8; j++) {
            r->coeffs[8 * i + j] = 0;   // 다항식의 한 계수에
            PQCLEAN_MLKEM768_CLEAN_cmov_int16(r->coeffs + 8 * i + j, ((KYBER_Q + 1) / 2), (msg[i] >> j) & 1); // coef, ( q+1 / 2 ), msg bit
        }
    }
}
...
void PQCLEAN_MLKEM768_CLEAN_ntt(int16_t r[256]) {
    ...
    for (len = 128; len >= 2; len >>= 1) {                  // 각 값의 쌍의 거리를 알려주는 index
        for (start = 0; start < 256; start = j + len) {     // 현재 Layer에 있는 group의 첫 시작 값을 
            zeta = PQCLEAN_MLKEM768_CLEAN_zetas[k++];       // 각 group에 맞는 zeta값을 가져와서 
...
void PQCLEAN_MLKEM768_CLEAN_indcpa_keypair_derand(uint8_t pk[KYBER_INDCPA_PUBLICKEYBYTES], uint8_t sk[KYBER_INDCPA_SECRETKEYBYTES], const uint8_t coins[KYBER_SYMBYTES]) 
{
    unsigned int i;
    ...
    for (i = 0; i < KYBER_K; i++) {
        PQCLEAN_MLKEM768_CLEAN_poly_getnoise_eta1(&skpv.vec[i], noiseseed, nonce++);    // -3 ~ 3 범위의 비밀키를 생성
    }   // 비밀키 s 생성
    for (i = 0; i < KYBER_K; i++) {
        PQCLEAN_MLKEM768_CLEAN_poly_getnoise_eta1(&e.vec[i], noiseseed, nonce++);   // -3 ~ 3 범위의 에러값 생성
    }   // 에러 다항식 e 생성

    PQCLEAN_MLKEM768_CLEAN_polyvec_ntt(&skpv);  // s를 NTT 변환
    PQCLEAN_MLKEM768_CLEAN_polyvec_ntt(&e);     // e를 NTT 변환
    ...
    PQCLEAN_MLKEM768_CLEAN_polyvec_add(&pkpv, &pkpv, &e);   // as + e
    PQCLEAN_MLKEM768_CLEAN_polyvec_reduce(&pkpv);           // mod q

    pack_sk(sk, &skpv);                                     // sk encode -> 바이트 배열로 저장
    pack_pk(pk, &pkpv, publicseed);                         // pk encode -> pk : A*s + e (768)  |  A행렬을 생성하는 seed값 (32)
}

void PQCLEAN_MLKEM768_CLEAN_indcpa_enc(uint8_t c[KYBER_INDCPA_BYTES], const uint8_t m[KYBER_INDCPA_MSGBYTES], const uint8_t pk[KYBER_INDCPA_PUBLICKEYBYTES], const uint8_t coins[KYBER_SYMBYTES]) 
{
    unsigned int i;
    ...
    PQCLEAN_MLKEM768_CLEAN_poly_frommsg(&k, m); // m의 값(message)을 k = m* 값(polynomial)로 변경, (Decompress_q(Decode_1(m), 1) 과정)
    ...
    PQCLEAN_MLKEM768_CLEAN_poly_getnoise_eta2(&epp, coins, nonce++); // e2 생성

    PQCLEAN_MLKEM768_CLEAN_polyvec_ntt(&sp); // r 행렬 NTT도메인으로 변경
    ...
    PQCLEAN_MLKEM768_CLEAN_polyvec_basemul_acc_montgomery(&v, &pkpv, &sp); // t * r

    PQCLEAN_MLKEM768_CLEAN_polyvec_invntt_tomont(&b);       // A * r 일반 다항식으로 변경
    PQCLEAN_MLKEM768_CLEAN_poly_invntt_tomont(&v);          // t * r 일반 다항식으로 변경

    PQCLEAN_MLKEM768_CLEAN_polyvec_add(&b, &b, &ep);        // A * r + e1
    PQCLEAN_MLKEM768_CLEAN_poly_add(&v, &v, &epp);          // t * r + e2
    PQCLEAN_MLKEM768_CLEAN_poly_add(&v, &v, &k);            // t * r + e2 + k*
...
void PQCLEAN_MLKEM768_CLEAN_indcpa_dec(uint8_t m[KYBER_INDCPA_MSGBYTES], const uint8_t c[KYBER_INDCPA_BYTES], const uint8_t sk[KYBER_INDCPA_SECRETKEYBYTES]) 
{
    polyvec b, skpv;
    poly v, mp;
    ...
    PQCLEAN_MLKEM768_CLEAN_polyvec_ntt(&b);         // hat_{u}
    PQCLEAN_MLKEM768_CLEAN_polyvec_basemul_acc_montgomery(&mp, &skpv, &b);  // s * u
    PQCLEAN_MLKEM768_CLEAN_poly_invntt_tomont(&mp);

    PQCLEAN_MLKEM768_CLEAN_poly_sub(&mp, &v, &mp);  // v - us
    PQCLEAN_MLKEM768_CLEAN_poly_reduce(&mp);        

    PQCLEAN_MLKEM768_CLEAN_poly_tomsg(m, &mp);  // encode (m*값을 m로 변경)
}
...
int PQCLEAN_MLKEM768_CLEAN_crypto_kem_keypair(uint8_t *pk, uint8_t *sk) 
{
    ...
    // uint8_t coins_temp[32] = {0x92, 0xAC, 0x7D, 0x1F, 0x83, 0xBA, 0xFA, 0xE6, 0xEE, 0x86, 0xFE, 0x00, 0xF9, 0x5D, 0x81, 0x33, 0x75, 0x77, 0x24, 0x34, 0x86, 0x0F, 0x5F, 0xF7, 0xD5, 0x4F, 0xFC, 0x37, 0x39, 0x9B, 0xC4, 0xCC};
    // for(int i = 0;i<32;i++)
    // {
    //     coins[i] = coins_temp[i];
    //     coins[i + 32] = coins_temp[i];
    // }

    
    PQCLEAN_MLKEM768_CLEAN_crypto_kem_keypair_derand(pk, sk, coins);    // ML-KEM internal 함수 호출
    return 0;
}
...
int PQCLEAN_MLKEM768_CLEAN_crypto_kem_enc_derand(uint8_t *ct, uint8_t *ss, const uint8_t *pk, const uint8_t *coins) 
{
    ...
    PQCLEAN_MLKEM768_CLEAN_indcpa_enc(ct, buf, pk, kr + KYBER_SYMBYTES); // ciphertext 생성, buf : ssk seed || H(pk),  pk : t || a seed,  kr + 32 : r = G(H(pk))
...
int PQCLEAN_MLKEM768_CLEAN_crypto_kem_enc(uint8_t *ct, uint8_t *ss, const uint8_t *pk) 
{
    ...
    // uint8_t coins_temp[KYBER_SYMBYTES] = {0x40, 0xBE, 0x9D, 0xCA, 0xC1, 0x6E, 0x9C, 0xA7, 0x3D, 0x49, 0xD0, 0xC8, 0x3F, 0x9D, 0x3D, 0x89, 0xBB, 0x71, 0x57, 0x4A, 0x42, 0x19, 0xA0, 0xF3, 0x93, 0xDF, 0xEC, 0xE2, 0x98, 0x83, 0x94, 0xC4};
...
int PQCLEAN_MLKEM768_CLEAN_crypto_kem_dec(uint8_t *ss, const uint8_t *ct, const uint8_t *sk) 
{
    ...
    PQCLEAN_MLKEM768_CLEAN_indcpa_dec(buf, ct, sk);         // buf에 message값 복호화
...
int PQCLEAN_MLKEM768_CLEAN_crypto_kem_keypair_KAT(uint8_t *pk, uint8_t *sk, uint8_t *d, uint8_t* z) 
{
    uint8_t coins[2 * KYBER_SYMBYTES];          // random 값 저장하기 위한 coin값 추가
    //randombytes(coins, 2 * KYBER_SYMBYTES);     // random 값 생성 d -> A 행렬 생성 seed, s 행렬 생성 seed 를 생성하는 seed, z -> implicit rejection을 위한 random 값
    memcpy(coins, d, 32);
...
int PQCLEAN_MLKEM768_CLEAN_crypto_kem_enc_KAT(uint8_t *ct, uint8_t *ss, const uint8_t *pk, uint8_t *ssk_seed) 
{
    uint8_t coins[KYBER_SYMBYTES];
    //randombytes(coins, KYBER_SYMBYTES); // SSK seed 값 생성
...
int PQCLEAN_MLKEM768_CLEAN_crypto_kem_dec_KAT(uint8_t *ss, const uint8_t *ct, const uint8_t *sk, uint8_t *invalid_ct, uint8_t *invalid_ss) 
{
    int fail;
    uint8_t buf[2 * KYBER_SYMBYTES];
    /* Will contain key, coins */
...
int TEST()
{
	...
	uint8_t* pk = (uint8_t*)malloc(PQCLEAN_MLKEM768_CLEAN_CRYPTO_PUBLICKEYBYTES);	//PQCLEAN_KYBER512_CLEAN_CRYPTO_PUBLICKEYBYTES -> 2 * 384 + 32
	uint8_t* sk = (uint8_t*)malloc(PQCLEAN_MLKEM768_CLEAN_CRYPTO_SECRETKEYBYTES);	//PQCLEAN_KYBER512_CLEAN_CRYPTO_SECRETKEYBYTES -> (2 * 384) + (2 * 384 + 32) + (2 * 32)
	uint8_t ct[KYBER_CIPHERTEXTBYTES];							//KYBER_CIPHERTEXTBYTES						   -> (2 * 320) + 128
	uint8_t ss[32];
	uint8_t ss2[32];

	PQCLEAN_MLKEM768_CLEAN_crypto_kem_keypair(pk, sk);			// KEY »ý¼º + KEY ±³È¯
...
int TEST_KAT_MLKEM()
{
    ...
    fp = fopen("kat_MLKEM_768.rsp", "r");

    while(feof(fp) == 0)
    {
        uint8_t pk[PQCLEAN_MLKEM768_CLEAN_CRYPTO_PUBLICKEYBYTES] = {0,};	//PQCLEAN_KYBER512_CLEAN_CRYPTO_PUBLICKEYBYTES -> 2 * 384 + 32
        uint8_t sk[PQCLEAN_MLKEM768_CLEAN_CRYPTO_SECRETKEYBYTES] = {0,};	//PQCLEAN_KYBER512_CLEAN_CRYPTO_SECRETKEYBYTES -> (2 * 384) + (2 * 384 + 32) + (2 * 32)
        ...
        uint8_t d_temp[32] = {0,};
        uint8_t msg[32] = {0,};
        uint8_t pk_temp[PQCLEAN_MLKEM768_CLEAN_CRYPTO_PUBLICKEYBYTES];
        uint8_t sk_temp[PQCLEAN_MLKEM768_CLEAN_CRYPTO_SECRETKEYBYTES];
        ...
        get_kat_parameter(fp, pk_temp, PQCLEAN_MLKEM768_CLEAN_CRYPTO_PUBLICKEYBYTES);
        get_kat_parameter(fp, sk_temp, PQCLEAN_MLKEM768_CLEAN_CRYPTO_SECRETKEYBYTES);
        ...
        PQCLEAN_MLKEM768_CLEAN_crypto_kem_keypair_KAT(pk, sk, d_temp, z_temp);
        if(memcmp(pk, pk_temp, PQCLEAN_MLKEM768_CLEAN_CRYPTO_PUBLICKEYBYTES)) return -1;
        if(memcmp(sk, sk_temp, PQCLEAN_MLKEM768_CLEAN_CRYPTO_SECRETKEYBYTES)) return -1;

        PQCLEAN_MLKEM768_CLEAN_crypto_kem_enc_KAT(ct, ss, pk, msg);
        
        if(memcmp(ct, ct_temp, KYBER_CIPHERTEXTBYTES)) return -1;
        if(memcmp(ss, ss_temp, 32)) return -1;

        PQCLEAN_MLKEM768_CLEAN_crypto_kem_dec_KAT(ss2, ct, sk, invalid_ct_temp, invalid_ss_temp);
