...
int randombytes(uint8_t *x, size_t xlen) {
	for (uint16_t cnt_i = 0; cnt_i < xlen; cnt_i++) x[cnt_i] = 0;
...
void KAT_indcpa_keypair(uint8_t pk[PUBLICKEY_BYTES],
...
void KAT_crypto_kem_keypair(uint8_t* pk, uint8_t* sk, uint8_t* keypair_seed, uint8_t* rejection_seed) {
    KAT_indcpa_keypair(pk, sk, keypair_seed);                             // PKE keygen 스킴을 통해 pk, sk 생성
...
int KAT_crypto_kem_enc(uint8_t* ctxt, uint8_t* ss, const uint8_t* pk, uint8_t* mu_seed) {
...
int main()
{
    ...
    uint8_t cmp_pk[PUBLICKEY_BYTES];
    ...
    while(feof(fp) == 0)
    {
        ...
        //printf("SMAUG-T KAT count : %d\n", count);

        ReadHex(fp, seed, 48, "seed = ");
        ReadHex(fp, keypair_seed, 32, "keypair_seed = ");
        ReadHex(fp, rejection_seed, 32, "rejection_seed = ");
        ReadHex(fp, cmp_pk, PUBLICKEY_BYTES, "pk = ");
        ReadHex(fp, cmp_sk, KEM_SECRETKEY_BYTES, "sk = ");
        ReadHex(fp, mu_seed, 32, "mu_seed = ");
        ReadHex(fp, cmp_ctxt, CIPHERTEXT_BYTES, "ct = ");
        ReadHex(fp, cmp_ss, 32, "ss = ");
        
        KAT_crypto_kem_keypair(pk, sk, keypair_seed, rejection_seed);
        if(memcmp(pk, cmp_pk, PUBLICKEY_BYTES)) return -1;
        if(memcmp(sk, cmp_sk, KEM_SECRETKEY_BYTES)) return -1;

        ch_val = KAT_crypto_kem_enc(ctxt, ss, pk, mu_seed);
        if(memcmp(ctxt, cmp_ctxt, CIPHERTEXT_BYTES)) return -1;

        ch_val = crypto_kem_dec(ss2, ctxt, sk);
        if(memcmp(ss, ss2, 32)) return -1;
        if(memcmp(ss, cmp_ss, 32)) return -1;
